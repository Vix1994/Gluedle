...
const homeMain = readFileSync(new URL("../src/main.ts", import.meta.url), "utf8");
const anchorNavigation = readFileSync(new URL("../src/anchor-wheel-navigation.ts", import.meta.url), "utf8");
const gameMain = readFileSync(new URL("../src/gluedle.ts", import.meta.url), "utf8");
const gamePresentation = readFileSync(
  new URL("../src/game/presentation.ts", import.meta.url),
  "utf8",
);
const gameBoardView = readFileSync(
  new URL("../src/game/game-board-view.ts", import.meta.url),
  "utf8",
);
const hintController = readFileSync(
  new URL("../src/game/hint-controller.ts", import.meta.url),
  "utf8",
);
const songSearchController = readFileSync(
  new URL("../src/game/song-search-controller.ts", import.meta.url),
  "utf8",
);
const catalogMain = readFileSync(new URL("../src/catalog.ts", import.meta.url), "utf8");
const editorialMain = readFileSync(new URL("../src/editorial.ts", import.meta.url), "utf8");
const appMain = readFileSync(new URL("../src/app.ts", import.meta.url), "utf8");
const appShell = readFileSync(new URL("../src/app-shell.ts", import.meta.url), "utf8");
...
const shareCardSource = readFileSync(new URL("../src/share/share-card.ts", import.meta.url), "utf8");
const browserShareSource = readFileSync(
  new URL("../src/share/browser-share.ts", import.meta.url),
  "utf8",
);
const resultShareController = readFileSync(
  new URL("../src/share/result-share-controller.ts", import.meta.url),
  "utf8",
);
const songCatalogLoader = readFileSync(new URL("../src/data/song-catalog.ts", import.meta.url), "utf8");
...
function elementTagById(source: string, tagName: string, id: string): string {
  const match = new RegExp(`<${tagName}\\b[^>]*\\bid="${id}"[^>]*>`, "i").exec(source);
  assert.ok(match, `expected <${tagName}>#${id}`);
  return match[0];
}

function importedStylePaths(source: string): string[] {
...
test("all six routes keep a persistent tab path, including the direct catalog route", () => {
  ...
  for (const source of [homeHtml, conceptHtml, visualsHtml, glueHtml, gameHtml, catalogHtml]) {
    ...
    assert.match(source, /type="module" src="\/src\/app\.ts"/);
  }
...
test("non-game routes share wheel navigation across explicit content anchors", () => {
  ...
  const routeAnchors: Array<[string, string[]]> = [
...
test("standalone game keeps informative suggestions, metadata clues, and image sharing", () => {
  ...
  const suggestionSource = extractFunction(songSearchController, "showSuggestions");
  assert.doesNotMatch(
    suggestionSource,
  ...
  assert.match(resultShareController, /renderShareCard\(elements\.shareCanvas, model\)/);
  ...
  assert.match(browserShareSource, /navigator\.canShare/);
  assert.match(resultShareController, /已自动保存图片/);
  assert.match(gameHtml, /data-result-reset/);
  assert.match(gameMain, /state = submitGuess\(state, selectedSong\.id, songs\);[\s\S]*?shares\.reset\(\);/);
  assert.doesNotMatch(gameMain, /renderAnswerReveal|answerReveal|resultShare/);
  assert.match(gameMain, /openResultDialog\(\);\s*void shares\.prepare\(\);/);
  assert.match(browserShareSource, /navigator\.canShare\(\{ files: \[file\] \}\)/);
  assert.match(browserShareSource, /downloadBlob\(blob, filename\)/);
  assert.match(hintController, /getHintStatus/);
  assert.match(hintController, /function revealNext/);
  assert.match(gameMain, /hints\.reset\(shuffleHints/);
});
...
test("game entry synchronizes progress, reloads JSON data, and randomizes each round", () => {
  ...
  const parsedCatalog = JSON.parse(songCatalogJson) as CatalogContractSong[];
  ...
  assert.match(songSearchController, /activeSuggestion\s*===\s*-1/);
  assert.match(
    songSearchController,
    /event\.key\s*===\s*["']ArrowDown["']\s*\?\s*0\s*:\s*suggestionSongs\.length\s*-\s*1/,
  );
  assert.match(songSearchController, /activeSuggestion\s*=\s*0;[\s\S]*?updateActiveSuggestion\(\)/);
  assert.match(songSearchController, /selectSuggestion\(song,\s*\{\s*submit:\s*true\s*\}\)/);
  const suggestionsSource = extractFunction(songSearchController, "showSuggestions");
  assert.match(suggestionsSource, /if\s*\(\s*!query\.trim\(\)\s*\)\s*\{[\s\S]*?closeSuggestions\(\);[\s\S]*?return;/);
  assert.match(suggestionsSource, /getSongs\(\)\.filter\([\s\S]*?!guessedIds\.has\(song\.id\)/);
...
test("responsive game ledger exposes every field without horizontal scrolling", () => {
  ...
  for (const field of ["favoriteCount", "language", "project", "performance", "originType", "featuredArtistGender", "credits"]) {
    assert.match(gameBoardView, new RegExp(`, "${field}",`));
  }
  assert.match(gameBoardView, /attempt\.comparison\.favoriteCount, "favoriteCount", "收藏数"/);
  assert.match(gameBoardView, /attempt\.comparison\.originType, "originType", "原唱\/翻唱"/);
  assert.match(gameBoardView, /attempt\.comparison\.featuredArtistGender, "featuredArtistGender", "合作对象"/);
  assert.match(gameBoardView, /cell\.colSpan = 8/);
  assert.match(gameBoardView, /appendSongComparisonCell\(row, song, attempt/);
  assert.match(gameBoardView, /formatReleaseDate\(comparison\.value\)/);
  ...
  assert.match(gameBoardView, /helper\.textContent = ""/);
  assert.match(gameBoardView, /cell\.dataset\.direction = comparison\.direction/);
  assert.match(gameBoardView, /value\.textContent = formattedValue/);
  ...
  assert.match(songSearchController, /dataset\.mobileBanner\s*=\s*"closed"/);
  assert.match(
    gameStyles,
...
test("result states use high-contrast colors plus non-color marks", () => {
  for (const [status, mark] of [["match", "✓"], ["near", "≈"], ["miss", "×"]]) {
    ...
    assert.ok(gamePresentation.includes(`${status}: "${mark}`));
  }
  assert.match(gameBoardView, /directionMark[\s\S]*?"▲"[\s\S]*?"▼"/);
  ...
  assert.match(gameBoardView, /createSongMetaItem\(attempt\.comparison\.year, "year", "发行日"\)/);
});
