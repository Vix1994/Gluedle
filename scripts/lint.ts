...
interface CatalogContractSong {
  language?: unknown;
  project?: { title?: unknown; type?: unknown } | null;
  curleyCredits?: { composition?: unknown; lyrics?: unknown } | null;
  hintLyrics?: unknown;
  [key: string]: unknown;
}
...
const typeScriptFiles = collectTypeScript(root);
const sourceTypeScriptFiles = collectTypeScript(join(root, "src"));
...
const homeMain = readFileSync(join(root, "src", "main.ts"), "utf8");
const anchorNavigation = readFileSync(join(root, "src", "anchor-wheel-navigation.ts"), "utf8");
const gameMain = readFileSync(join(root, "src", "gluedle.ts"), "utf8");
const gamePresentation = readFileSync(join(root, "src", "game", "presentation.ts"), "utf8");
const songSearchController = readFileSync(
  join(root, "src", "game", "song-search-controller.ts"),
  "utf8",
);
const catalogMain = readFileSync(join(root, "src", "catalog.ts"), "utf8");
const editorialMain = readFileSync(join(root, "src", "editorial.ts"), "utf8");
const appMain = readFileSync(join(root, "src", "app.ts"), "utf8");
const appShell = readFileSync(join(root, "src", "app-shell.ts"), "utf8");
...
const shareCardSource = readFileSync(join(root, "src", "share", "share-card.ts"), "utf8");
const browserShareSource = readFileSync(join(root, "src", "share", "browser-share.ts"), "utf8");
const resultShareController = readFileSync(
  join(root, "src", "share", "result-share-controller.ts"),
  "utf8",
);
const songCatalogSource = readFileSync(join(root, "src", "data", "song-catalog.ts"), "utf8");
...
const releaseBoundaryMarkers: Array<[string, string, string, string]> = [
...
if (
  ...
  errors.push("src/game/song-search-controller.ts: suggestions must reveal song titles and metadata safely");
}

if (
  !suggestionsSource
  || !/if\s*\(\s*!query\.trim\(\)\s*\)\s*\{\s*closeSuggestions\(\);\s*return;\s*\}/.test(suggestionsSource)
  || !/getSongs\(\)\.filter\([\s\S]*?!guessedIds\.has\(song\.id\)[\s\S]*?\)/.test(suggestionsSource)
) {
  errors.push("src/game/song-search-controller.ts: empty input must close suggestions and exclude guessed songs");
}

const htmlForbidden: Array<[RegExp, string]> = [
...
if (!homeHtml.includes('type="module" src="/src/app.ts"')) {
  errors.push("index.html: missing Vite module entry");
}
if (!gameHtml.includes('type="module" src="/src/app.ts"')) {
...
if (
  !catalogHtml.includes('type="module" src="/src/app.ts"')
...
for (const { label, source } of editorialHtmlFiles) {
  if (
    !source.includes('type="module" src="/src/app.ts"')
...
const editorialAnchorClasses: Array<[string, string[]]> = [
...
const parsedSongCatalog = JSON.parse(songCatalogJson) as CatalogContractSong[];
if (
  ...
  || parsedSongCatalog.some((song) =>
    typeof song.language !== "string" || !["zh", "en"].includes(song.language))
...
if (
  ...
  errors.push("src/gluedle.ts: each new round must select a random answer without daily state");
}
...
if (
  ...
  errors.push("src/gluedle.ts: rendered progress must synchronize data attributes, CSS progress, and markers");
}

if (
  !/activeSuggestion\s*===\s*-1/.test(songSearchController)
  || !/event\.key\s*===\s*["']ArrowDown["']\s*\?\s*0\s*:\s*suggestionSongs\.length\s*-\s*1/
    .test(songSearchController)
  || !/activeSuggestion\s*=\s*0;[\s\S]*?updateActiveSuggestion\(\)/.test(suggestionsSource ?? "")
  || !/selectSuggestion\(song,\s*\{\s*submit:\s*true\s*\}\)/.test(songSearchController)
) {
  errors.push("src/game/song-search-controller.ts: search must default-select and submit candidates directly");
}
...
if (
  !resultShareController.includes("renderShareCard(elements.shareCanvas, model)")
  || !browserShareSource.includes("navigator.canShare({ files: [file] })")
  || !browserShareSource.includes("downloadBlob(blob, filename)")
...
for (const [status, mark] of [["match", "✓"], ["near", "≈"], ["miss", "×"]]) {
  if (
    ...
    || !gamePresentation.includes(`${status}: "${mark}`)
  ) {
...
for (const color of ["#050505", "#f4f3ed", "#87a8be", "#49e99b", "#ffd75b", "#ff5964"]) {
  if (!shareCardSource.toLowerCase().includes(color)) {
    errors.push(`src/share/share-card.ts: missing restrained share-card color ${color}`);
  }
}
if (/#8e2638/i.test(shareCardSource)) {
  errors.push("src/share/share-card.ts: saturated legacy miss color must not return");
}
...
if (viteInputs.some((input) => !viteConfig.includes(`"${input}"`))) {
  errors.push("vite.config.ts: production build must include all six HTML entries");
}
...
if (errors.length) {
  ...
  console.log(`Lint passed: ${typeScriptFiles.length} TypeScript files and six-page HTML contracts checked.`);
}
