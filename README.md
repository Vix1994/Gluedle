...
Gluedle 是一款无音频的每日歌曲资料推理游戏，也是一座围绕 **GLUE / Green to Blue** 视觉概念展开的可浏览网站。玩家从曲库选择歌曲，在最多 6 次尝试内，根据年份、时长、所属项目、语言、演唱类型与创作资料的比较反馈找到每日答案。
...
- `#concept`：涟漪、轨道和概念说明；
- `#story`：编辑式影像拼贴与由曲库直接渲染的曲目资料入口；
- `#gluedle`：每日无音频元数据推理游戏；
- `#credits`：资料来源、素材状态与版权边界。

## 游戏规则

1. 搜索曲库并从建议中选择一首歌；
2. 提交后比较歌曲、年份、时长、所属项目、语言、演唱与创作资料；
...
├── src/data/catalog.js      # 页面文案、歌曲目录与数据声明
...
曲目目录内的来源链接与待核验字段以 `src/data/catalog.js` 为准。视觉素材的授权状态仍需在公开发布前确认；未知创作署名保持为 `null` / “待核验”，不会推测或补写。
