...
type LocationInput = URL | Location | string | { href?: unknown };

export function createQrMatrix(text: unknown): QrMatrix {
  ...
  let best: { matrix: MutableQrMatrix; score: number } | null = null;

  for (let mask = 0; mask < 8; mask += 1) {
    const matrix = createBaseMatrix();
    placeCodewords(matrix, codewords, mask);
    placeFormatBits(matrix, mask);
    const score = penaltyScore(matrix as QrMatrix);
    if (!best || score < best.score) best = { matrix, score };
  }

  if (!best) throw new Error("Unable to create QR matrix.");
  return best.matrix as QrMatrix;
}

export function canonicalGameUrl(locationLike: LocationInput): string {
  const href = typeof locationLike === "object" && locationLike !== null && "href" in locationLike
    ? locationLike.href
    : locationLike;
...
function createDataCodewords(bytes: number[]): number[] {
  const bits: number[] = [];
...
function createBaseMatrix(): MutableQrMatrix {
  const matrix: MutableQrMatrix = Array.from(
    { length: QR_SIZE },
...
function placeFinder(matrix: MutableQrMatrix, left: number, top: number): void {
  for (let y = -1; y <= 7; y += 1) {
    for (let x = -1; x <= 7; x += 1) {
      const row = top + y;
      const column = left + x;
...
function placeAlignment(matrix: MutableQrMatrix, centerX: number, centerY: number): void {
...
function reserveFormatAreas(matrix: MutableQrMatrix): void {
...
function placeCodewords(matrix: MutableQrMatrix, codewords: number[], mask: number): void {
...
function placeFormatBits(matrix: MutableQrMatrix, mask: number): void {
