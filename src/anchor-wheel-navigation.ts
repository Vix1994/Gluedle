export function setupAnchorWheelNavigation({ signal }: { signal?: AbortSignal } = {}) {
  ...
  const anchorTop = (anchor: HTMLElement): number => {
  ...
  const nextAnchor = (direction: number): HTMLElement | null => {
  ...
  const nestedScrollerCanMove = (target: EventTarget | null, direction: number): boolean => {
