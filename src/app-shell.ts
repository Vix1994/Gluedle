type ControllerName = "home" | "editorial" | "gluedle" | "catalog";
...
export function createAppShell(mountController: (name: ControllerName) => (() => void) | void) {
  const host = document.querySelector<HTMLElement>("[data-app-header]");
  const initialRouteView = document.querySelector<HTMLElement>("[data-route-view]");
  const initialRouteStyle = findRouteStyle(document);
  if (!host || !initialRouteView || !initialRouteStyle) {
    throw new Error("The shared app shell is incomplete.");
  }
  let routeView: HTMLElement = initialRouteView;
  let routeStyle: HTMLLinkElement = initialRouteStyle;

  host.innerHTML = headerMarkup();
  const header = host.querySelector<HTMLElement>("[data-app-header-element]");
  const action = host.querySelector<HTMLButtonElement>("[data-app-action]");
  const progress = host.querySelector<HTMLElement>("[data-app-progress]");
  if (!header || !action || !progress) throw new Error("The shared app header is incomplete.");
  ...
  const updateHeader = () => {
    ...
    host.querySelectorAll<HTMLAnchorElement>(".app-nav a").forEach((link) => {
  ...
  const activateController = () => {
    currentCleanup();
    const cleanup = mountController(currentRoute().controller);
  ...
  const loadStyle = (href: string) => new Promise<() => void>((resolve, reject) => {
    ...
    const nextStyle = routeStyle.cloneNode() as HTMLLinkElement;
    nextStyle.href = href;
  ...
  const swapRoute = async (destination: URL, push = true) => {
    ...
    try {
      ...
      const nextView = parsed.querySelector<HTMLElement>("[data-route-view]");
      ...
      const nextStyleHref = nextStyle.getAttribute("href");
      if (!nextStyleHref) throw new Error("Route stylesheet is missing its href.");
      const activateStyle = await loadStyle(new URL(nextStyleHref, destination.href).href);
      ...
      const commit = () => {
        ...
        activateController();
        document.querySelector<HTMLElement>("[data-route-view] main")?.focus({ preventScroll: true });
      };
...
function syncDocument(parsed: Document) {
...
function findRouteStyle(documentNode: Document): HTMLLinkElement | null {
  return documentNode.querySelector<HTMLLinkElement>("[data-route-style]")
    ?? [...documentNode.querySelectorAll<HTMLLinkElement>('link[rel="stylesheet"]')].at(-1)
    ?? null;
}
