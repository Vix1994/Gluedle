import type { SongCredits } from "../types.ts";
...
export function extractCurleyCredits(lyric: unknown): SongCredits {
...
function cleanCreditLine(line: unknown): string {
  return String(line ?? "")
    .replace(/^(?:\[[^\]]+\])+/u, "")
    .trim();
}

function creditIncludesCurley(lines: readonly string[], pattern: RegExp): boolean {
