...
import type { FeaturedArtistGender } from "../types.ts";
...
const MANUAL_FEATURED_ARTIST_GENDERS: Readonly<Record<string, FeaturedArtistGender>> = Object.freeze({
  "Ari Abdul": "female",
  "Cody Fry": "male",
...
export function getFeaturedArtistGender(featuredArtists: unknown): FeaturedArtistGender {
  if (!Array.isArray(featuredArtists) || featuredArtists.length === 0) {
    return "none";
  }

  const genders: FeaturedArtistGender[] = featuredArtists.map((artist) => (
